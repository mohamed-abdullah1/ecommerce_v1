...
export default function Orders() {
  ...
  const {
    ...
    error: ordersError,
  } = useFetch("http://localhost:9898/api/orders/", {
    headers: { token: `Bearer ${accessToken}` },
  });
  //functions
  const createRow = (orderObj) => {
    ...
    return {
      ...
      address,
      status,
      productDetails: productsInOrder.map(({ productId, quantity }) => ({
        productName:
          products?.find((product) => productId === product._id)?.title ||
          "can't be fetched",
        productId,
        quantity,
        totalPrice:
          products?.find((product) => productId === product._id)?.price *
            quantity || 0,
      })),
    };
  };
  //useEffects
  useEffect(() => {
    console.log("orders", fetchedOrders, "products", products);
    console.log(
      "orders loading",
      ordersLoading,
      "products loading",
